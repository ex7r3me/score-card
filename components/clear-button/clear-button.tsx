...
export default function VerificationDialog({ onAccept }: { onAccept: () => void }) {
  ...
  return (
    <>
      <Button onPress={() => setIsOpen(true)} className='inline-block bg-accent-gray text-white py-1 px-3 font-bold rounded-md' aria-label='clear board'>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className='fixed z-30 inset-0 flex items-center justify-center'>
        <Dialog.Panel className='bg-light-green p-4 rounded-2xl flex flex-col items-center '>
          ...
          <div>
            <Button onPress={() => handleSelection(true)} className='bg-dark-green text-white py-1 px-3 font-bold rounded-md mr-3'>Yes</Button>
            <Button onPress={() => handleSelection(false)} className='bg-accent-gray text-white py-1 px-3 font-bold rounded-md'>No</Button>
          </div>
        </Dialog.Panel>
      </Dialog>
    </>
